...
func NewRequest(method string, url string, header http.Header, body interface{}) (req *http.Request, err error) {
    var r io.Reader
    if body != nil {
        switch v := body.(type) {          //类型转换这一步是关键 只能写成 v := ...(type)
        case io.Reader:
            r = v
        case string:
...
func DoRequest(req *http.Request, expectation interface{}) error {
    ...
    if err != nil {
        return fmt.Errorf("do request errors: %s", err)
    }
